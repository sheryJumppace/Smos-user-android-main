...
private fun getAddedUsers(
    previousUsersList: List<QBUser>,
    currentUsersList: List<QBUser>
): List<QBUser> {
...
fun getOccupantsIdsListFromString(occupantIds: String?): List<Int> {
    ...
    occupantIds?.let {
        val occupantIdsArray =
            occupantIds.split(",".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
