package com.smox.smoxuser.utils.chat
...
import com.smox.smoxuser.*
import com.smox.smoxuser.utils.SharedPrefsHelper
import com.smox.smoxuser.utils.qb.*
import com.smox.smoxuser.utils.qb.callback.QbEntityCallbackTwoTypeWrapper
...
object ChatHelper {
    ...
    init {
        QBSettings.getInstance().logLevel = LogLevel.NOTHING
    ...
    fun getCurrentUser(): QBUser? {
        try {
            val savedQBUser: QBUser? = SharedPrefsHelper.getQbUser()!!
            val qbCurrentUser = QBChatService.getInstance().user
            if (qbCurrentUser != null) {
                return qbCurrentUser
            } else {
                return savedQBUser
            }
        } catch (e: Exception) {
        ...
        //Log.i("-curUser-", savedQBUser!!.fullName.toString())
    ...
    fun join(dialogs: List<QBChatDialog>) {
        for (dialog in dialogs) {
            ...
            try {
                ...
                Log.e("-ChatJoin-", e.message.toString())
            }
    ...
    fun deleteDialogs(
        dialogs: Collection<QBChatDialog>,
    ...
    fun getDialogs(
        customObjectRequestBuilder: QBRequestGetBuilder,
    ...
    private fun getUsersFromDialogs(
        ...
        QBUsers.getUsersByIDs(userIds, requestBuilder).performAsync(
            object :
                QbEntityCallbackTwoTypeWrapper<ArrayList<QBUser>, ArrayList<QBChatDialog>>(callback) {
    ...
    private fun getUsersFromMessages(
        ...
        QBUsers.getUsersByIDs(userIds, requestBuilder).performAsync(
            object :
                QbEntityCallbackTwoTypeWrapper<ArrayList<QBUser>, ArrayList<QBChatMessage>>(callback) {
                override fun onSuccess(t: ArrayList<QBUser>, bundle: Bundle?) {
                    QbUsersHolder.putUsers(t)
