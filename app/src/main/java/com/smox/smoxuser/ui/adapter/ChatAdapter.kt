...
import com.smox.smoxuser.utils.managers.PROPERTY_NOTIFICATION_TYPE
import com.smox.smoxuser.utils.qb.PaginationHistoryListener
...
class ChatAdapter(
    ...
    fun removeMessage(position: Int) {
        this.notifyItemRemoved(chatMessages.size - 1)
        ...
        when (viewType) {
            TYPE_NOTIFICATION_CENTER -> {
                return NotificationHolder(
                    inflater.inflate(
                        containerLayoutRes.get(viewType), parent,
                        false
                    ), R.id.msg_text_message, R.id.msg_text_time_message
                )
            }
            TYPE_TEXT_RIGHT -> {
            ...
            TYPE_ATTACH_RIGHT -> {
                return ImageAttachHolder(
                    inflater.inflate(
                        containerLayoutRes.get(viewType), parent,
                        false
                    ), R.id.msg_image_attach, R.id.msg_progressbar_attach,
    ...
    override fun onViewRecycled(holder: MessageViewHolder) {
        ...
        if (containerLayoutRes.get(holder.itemViewType) != 0 && holder.avatar != null) {
            Glide.with(holder.avatar!!.context).clear(holder.avatar!!)
        }
    ...
    private fun onBindViewMsgLeftHolder(holder: TextMessageHolder, chatMessage: QBChatMessage) {
        ...
        val customMessageTimeTextView =
            holder.itemView.findViewById<TextView>(R.id.custom_msg_text_time_message)
    ...
    private fun fillTextMessageHolder(
        ...
        if (urlsList.isEmpty()) {
            ...
            holder.messageTextView.maxWidth =
                context.resources.getDimension(R.dimen.link_preview_width).toInt()
    ...
    private fun setOpponentsName(
        holder: MessageViewHolder,
        chatMessage: QBChatMessage,
        isAttachment: Boolean
    ) {
        val viewId = R.id.opponent_name_attach_view
   ...
   /* override fun getHeaderId(position: Int): Long {
        val chatMessage = getItem(position)
        //Log.d(TAG, "${chatMessage.dateSent * 1000}")
   ...
   /* override fun onBindHeaderViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
    ...
    private fun getDate(seconds: Long): String {
        ...
        neededTime.timeInMillis = seconds * 1000

        //Log.d(TAG, " Now Time:- ${nowTime.get(Calendar.DATE)} , Needed Time:- ${neededTime.get(Calendar.DATE)}")
        return when {
            nowTime.get(Calendar.DATE) == neededTime.get(Calendar.DATE) -> "Today"
            nowTime.get(Calendar.DATE) - neededTime.get(Calendar.DATE) == 1 &&
                    nowTime.get(Calendar.MONTH) == neededTime.get(Calendar.MONTH) -> "Yesterday"
            else -> dateFormat.format(Date(seconds * 1000))//here return like "May 31"
        }
    }
    ...
    private fun showImageByURL(holder: MessageViewHolder, url: String) {
        val preferredImageWidth =
            context.resources.getDimension(R.dimen.attach_image_width_preview).toInt()
        val preferredImageHeight =
            context.resources.getDimension(R.dimen.attach_image_height_preview).toInt()
    ...
    private fun getRequestListener(holder: MessageViewHolder): RequestListener<Drawable> {
    ...
    private inner class ImageLoadListener<P>(val holder: ImageAttachHolder) : RequestListener<P> {
    ...
    private inner class ItemClickListenerFilter internal constructor(
        private val chatAttachClickListener:
        AttachClickListener,
