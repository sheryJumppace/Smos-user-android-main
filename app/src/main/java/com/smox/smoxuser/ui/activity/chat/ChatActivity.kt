...
import com.smox.smoxuser.ui.adapter.AttachmentPreviewAdapter
import com.smox.smoxuser.ui.adapter.ChatAdapter
import com.smox.smoxuser.ui.dialog.ProgressDialogFragment
import com.smox.smoxuser.ui.views.AttachmentPreviewAdapterView
import com.smox.smoxuser.utils.ACTION_REFRESH_CHAT
import com.smox.smoxuser.utils.SystemPermissionHelper
import com.smox.smoxuser.utils.chat.CHAT_HISTORY_ITEMS_PER_PAGE
import com.smox.smoxuser.utils.chat.ChatHelper
import com.smox.smoxuser.utils.imagepick.OnImagePickedListener
import com.smox.smoxuser.utils.imagepick.pickAnImage
import com.smox.smoxuser.utils.listeners.AttachClickListener
import com.smox.smoxuser.utils.managers.DialogsManager
import com.smox.smoxuser.utils.qb.*
...
class ChatActivity : BaseActivity(), OnImagePickedListener, QBMessageStatusListener,
    ...
    private lateinit var sendClick: ImageButton
    ...
    public override fun onCreate(savedInstanceState: Bundle?) {
        ...
        supportActionBar?.apply {
            ...
            //setHomeAsUpIndicator(ContextCompat.getDrawable(this@ChatActivity, Constants.backButton))
        }
    ...
    override fun onBackPressed() {
        ...
        if (fromNotification) {
            val chatIntent = Intent(ACTION_REFRESH_CHAT)
            chatIntent.putExtra(EXTRA_DIALOG_ID, qbChatDialog.dialogId)
    ...
    private fun loadUsersFromQb(qbChatDialog: QBChatDialog) {
        ...
        QBUsers.getUsers(qbPagedRequestBuilder)
            .performAsync(object : QBEntityCallback<java.util.ArrayList<QBUser>> {
                override fun onSuccess(users: ArrayList<QBUser>, params: Bundle) {
                    ...
                    if (qbChatDialog.occupants.size >= users.size) {
                        ...
                        GroupActivity.startForResult(
                            this@ChatActivity,
                            REQUEST_CODE_SELECT_PEOPLE,
                            qbChatDialog
                        )
                    }
                }
    ...
    private fun leaveGroupChat() {
        ...
        ChatHelper.exitFromDialog(qbChatDialog, object : QBEntityCallback<QBChatDialog> {
            ...
            override fun onError(e: QBResponseException) {
                ...
                hideProgressDialog()
                showErrorSnackbar(
                    R.string.error_leave_chat,
                    e,
                    View.OnClickListener { leaveGroupChat() })
            }
        })
    }
    ...
    override fun onRequestPermissionsResult(
        ...
        if (requestCode == PERMISSIONS_FOR_SAVE_FILE_IMAGE_REQUEST && requestCode == PERMISSIONS_FOR_CAMERA_REQUEST && grantResults[0] != -1) {
            openImagePicker()
        }
    }
    ...
    override fun onImagePicked(requestCode: Int, file: File) {
        when (requestCode) {
            REQUEST_CODE_ATTACHMENT -> {
                ...
                showProgressDialog(R.string.wait_a_minute)
                ChatHelper.loadFileAsAttachment(file, object : QBEntityCallback<QBAttachment> {
                    override fun onSuccess(result: QBAttachment, params: Bundle?) {
                        _fileQBAttachmentMap[file] = result
                        hideProgressDialog()
                        sendClick.performClick()
                    }
    ...
    fun onSendChatClick(view: View) {
        ...
        if (uploadedAttachments.isNotEmpty()) {
            if (uploadedAttachments.size == _fileQBAttachmentMap.size) {
    ...
    private fun initViews() {
        ...
        sendClick = findViewById(R.id.button_chat_send)
        ...
        val previewAdapterView =
            findViewById<AttachmentPreviewAdapterView>(R.id.adapter_view_attachment_preview)
    ...
    private fun openImagePicker() {
        ...
        } else if (!permissionHelper.isCameraPermissionGranted()) {
            permissionHelper.requestPermissionsForCamera()
        } else {
    ...
    private fun getQBUsersName(newUsersIds: List<Int>) {
        QBUsers.getUsersByIDs(newUsersIds, null)
            .performAsync(object : QBEntityCallback<ArrayList<QBUser>> {
                override fun onSuccess(qbUsers: ArrayList<QBUser>, bundle: Bundle) {
                    ...
                    dialogsManager.sendMessageCreatedDialog(qbChatDialog, usersNames)
    ...
    private fun notifyUsersAboutCreatingDialog() {
        if (intent.getBooleanExtra(EXTRA_IS_NEW_DIALOG, false)) {
            ...
            getQBUsersName(qbChatDialog.occupants)
    ...
    private fun updateDialog(selectedUsers: ArrayList<QBUser>) {
        ChatHelper.updateDialogUsers(
            ...
            object : QBEntityCallback<QBChatDialog> {
                ...
                override fun onError(e: QBResponseException) {
                    showErrorSnackbar(
                        R.string.chat_info_add_people_error,
                        e,
                        View.OnClickListener { updateDialog(selectedUsers) })
                }
            })
    }
    ...
    private fun loadDialogUsers() {
        ChatHelper.getUsersFromDialog(qbChatDialog, object : QBEntityCallback<ArrayList<QBUser>> {
            ...
            override fun onError(e: QBResponseException) {
                showErrorSnackbar(
                    R.string.chat_load_users_error,
                    e,
                    View.OnClickListener { loadDialogUsers() })
            }
        })
    }
    ...
    private fun setChatNameToActionBar() {
        ...
        //supportActionBar?.setHomeAsUpIndicator(ContextCompat.getDrawable(this, Constants.backButton))
    }
    ...
    private fun deleteChat() {
        ChatHelper.deleteDialog(qbChatDialog, object : QBEntityCallback<Void> {
            ...
            override fun onError(e: QBResponseException) {
                showErrorSnackbar(
                    R.string.dialogs_deletion_error,
                    e,
                    View.OnClickListener { deleteChat() })
            }
        })
    }
